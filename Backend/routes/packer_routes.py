...
# @packer_bp.route('', methods=['POST'])
# def create_packer():
#     """Create a new packer in the SQLite database"""
#     conn = get_db_connection()
...
#             int(data.get('spouts', 8)), 
#             float(data.get('rpm', 5)), 
#             data.get('camera_id'), 
#             float(data.get('line_position', 0.7)),
#             float(data.get('start_line_position', 0.2)), 
...
#     query = f"UPDATE packers SET {', '.join(updates)} WHERE id = ?"
...
#     if packer['status'] == 'active':
#         conn.close()
#         return jsonify({"error": "Cannot delete active packer"}), 400
